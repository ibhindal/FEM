...
def strainDisp2D(SF,nodeCoor,Jacob) : 
    ...
    I = Jacob['invJ'] 
    r1 = c_[dsfdx,np.zeros(nne)].flatten() 
    r2 = c_[dsfde,np.zeros(nne)].flatten() 
    R = I.dot(c_[r1, r2].T) 
    
    dudx = R[0,:] 
    dudy = R[1,:] 
    r1 = c_[np.zeros(nne),dsfdx].flatten()
    r2 = c_[np.zeros(nne),dsfde].flatten() 
    R = I.dot(c_[r1, r2].T) 
    dvdx = R[0,:] 
    dvdy = R[1,:] 
    B = c_[dudx, dvdy, dudy+dvdx].T 
    return B